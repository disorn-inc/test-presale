...
export const useUnstakeTokens = (tokenAddress: string) => {
    ...
    const tokenFarmAddress = chainId ? networkMapping[String(chainId)]["PresaleTest"][0] : constants.AddressZero;
    ...
    const {send: unstakeSend, state: unstakeState} = useContractFunction(
        tokenFarmContract, "claimToken", {transactionName: "Unstake Tokens"}
    );
